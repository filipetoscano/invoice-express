...
foreach ( var country in all!.OrderBy( x => x.Alpha2 ) )
{
    ...
    sb2.AppendLine( $"        new IXCountry( \"{ country.Alpha2 }\", \"{ ieName }\" )," );
}
...
{
    await File.WriteAllTextAsync( "v1-" + args[ 0 ], sb.ToString() );
    await File.WriteAllTextAsync( "v2-" + args[ 0 ], sb2.ToString() );
}
